...
def extract_features(imgs, feature_fns, verbose=False):
    ...
    这段代码是一个批量特征提取器，用来把 N 张图像 通过 多个特征函数 提取出 统一的特征矩阵。
    输入：N 张图片 + 多个特征函数
    输出：一个 (N, 总特征维度) 的矩阵，每行是一张图的所有特征拼接。
...
def hog_feature(im):
    ...
    gx[:, :-1] = np.diff(image, n=1, axis=1)  # compute gradient on x-direction # 差分 边缘
    gy[:-1, :] = np.diff(image, n=1, axis=0)  # compute gradient on y-direction
    grad_mag = np.sqrt(gx ** 2 + gy ** 2)  # gradient magnitude
    grad_ori = np.arctan2(gy, (gx + 1e-15)) * (180 / np.pi) + 90  # gradient orientation # 四象限正切 1e-15防止无穷
    ...
    for i in range(orientations):
        ...
        # isolate orientations in this range  # 三目
