...
def rnn_step_backward(dnext_h, cache):
    ...
    (x, prev_h, Wx, Wh, b, y4)= cache
    dy3= dnext_h * (1 - y4**2)

    db= np.sum(dy3, axis=0)
    dx= np.dot(dy3, Wx.T)
    dWx= np.dot(x.T, dy3)
    dprev_h= np.dot(dy3, Wh.T)
    dWh= np.dot(prev_h.T, dy3)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        next_h, cache_i= rnn_step_forward(x[:,i,:], pre_h, Wx, Wh, b)
        h[:,i,:]= next_h
        cache[i]= cache_i
        pre_h= next_h
...
def rnn_backward(dh, cache):
    ...
    (N, T, D, H)= cache[-1]
    dx= np.zeros((N, T, D))
    dh_add= np.zeros((N, H)) # 用来累加，之后的 h 对 loss 的贡献
    dWx= np.zeros((D, H))
    dWh= np.zeros((H, H))
    db= np.zeros((H,))

    for i in range(T-1, -1, -1):
        dx[:,i,:], dh_add, dWx_i, dWh_i, db_i= rnn_step_backward(dh[:,i,:]+dh_add, cache[i])
        dWx+= dWx_i
        dWh+= dWh_i
        db+= db_i
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    ig= sigmoid( Y[:,:H] )
    fg= sigmoid( Y[:,H:2*H] )
    og= sigmoid( Y[:,2*H:3*H] )
    gg= np.tanh( Y[:,3*H:4*H] )

    next_c= ig*gg + fg*prev_c
    next_h= np.tanh(next_c) * og

    cache= (x, prev_h, prev_c, Wx, Wh, b, N, H, Y, ig, fg, og, gg, next_c, np.tanh(next_c), next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    (x, prev_h, prev_c, Wx, Wh, b, N, H, Y, ig, fg, og, gg, next_c, next_c_, next_h)= cache

    dog= dnext_h*next_c_
    dnext_c_= dnext_h*og
    dnext_c1= (1 - next_c_**2) * dnext_c_
    dnext_c+= dnext_c1
    ...
    dY= np.zeros(Y.shape)
    dY[:,:H]= dig * (ig - ig**2)
    dY[:,H:2*H]= dfg * (fg - fg**2)
    dY[:,2*H:3*H]= dog * (og - og**2)
    dY[:,3*H:4*H]= dgg * (1 - gg**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h, next_c, cache[t]= lstm_step_forward(x[:,t,:], h0, c0, Wx, Wh, b)
        h[:,t,:]= next_h
        cache_c[t]= next_c
        h0= next_h
        c0= next_c
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        dx[:,t,:], dh_add, dc, dWx_i, dWh_i, db_i= lstm_step_backward(dh[:,t,:]+dh_add, dc, cache[t])
        dWx+= dWx_i
        dWh+= dWh_i
        db+= db_i
